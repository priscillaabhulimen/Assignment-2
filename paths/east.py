...
def handle_shack(state, continue_game):
    typewriter_print("You enter the shack. It is dark and dusty.", state=state)
    ...
    if choice == "Look around":
        ...
        if choice == "Read the book":
            ...
            typewriter_print("You decide to rest a bit. The throbbing in your ankle slowly eases.", state=state)
...
def esc_sequence(state, continue_game):
    typewriter_print("Suddenly, you hear a sound from outside. Your heart leaps into your throat.", state=state)
    ...
    if choice == 'Investigate':
        ...
        trapped_in_town('shack', state)
    else:
        handle_stay_inside(state, continue_game)


def handle_stay_inside(state, continue_game):
    typewriter_print("You rush and manage to click the old lock on the door shut.", state=state)
    delayed_print("\nSLAM!")
    typewriter_print("Something has started to ram into the door. The wood is splintering. It bursts open", state=state)
